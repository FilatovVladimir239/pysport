...
Экспорт протоколов TrailO в **Excel**: плагин SportOrg и отдельное окно для файлов `.json` / `.json.gz`.
...
| Поле | Назначение |
|------|------------|
| `output_dir` | Папка для `.xlsx` (пусто — `Documents/TrailOProtocols`) |
...
`trailo_protocol.py` и расчёт TrailO — в пакете `sportorg`. Запись `.xlsx` — `trailo_protocols/excel.py`.
